...
export const HeroSection: React.FC<HeroSectionProps> = ({
  ...
  return (
    <div className={`mb-10 w-full max-w-2xl mx-auto transition-all duration-500 ${domainCount > 0 ? 'scale-95' : 'scale-100'}`}>
      ...
      <div className="relative group z-10">
        ...
        <div className="relative flex items-center bg-zinc-900/90 backdrop-blur-md rounded-xl shadow-lg border border-zinc-800 p-2 min-w-0">
          ...
          <input
            ...
            className="min-w-0 flex-1 px-3 sm:px-4 py-3 text-base sm:text-lg bg-transparent border-none focus:ring-0 text-white placeholder:text-zinc-600 font-medium"
            value={newDomainUrl}
            onChange={(e) => {
          ...
          />
          <div className="flex shrink-0 items-center gap-1 sm:gap-1.5 mr-1">
            <button
              onClick={onShowBulkImport}
              className="text-zinc-500 hover:text-emerald-400 px-2 sm:px-3 py-2 text-sm font-medium transition-colors"
              title="Bulk Import"
            >
              Bulk
            </button>
            <button
              ...
              className="bg-emerald-500 hover:bg-emerald-400 text-white px-4 sm:px-6 py-2.5 rounded-lg font-medium transition-all shadow-md active:transform active:scale-95 shadow-emerald-500/20"
            >
              Track
            </button>
          </div>
