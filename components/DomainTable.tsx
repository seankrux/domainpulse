...
const StatusBadge: React.FC<{ status: DomainStatus; statusCode?: number }> = ({
  status,
  statusCode,
}) => {
  const configs = {
    [DomainStatus.Alive]: {
      ...
      dot: "bg-emerald-500 status-dot-green",
      border: "border-emerald-500/20",
    },
    [DomainStatus.Down]: {
      bg: "bg-red-500/10",
      text: "text-red-400",
      dot: "bg-red-500 status-dot-red",
...
const SSLBadge: React.FC<{ ssl?: SSLInfo; onClick?: () => void }> = ({
  ssl,
  onClick,
}) => {
...
// Per-domain uptime badge
const UptimeBadge: React.FC<{ history: { status: DomainStatus }[] }> = ({
  history,
}) => {
  if (history.length === 0)
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-mono font-semibold border ${color} ${bg}`}
...
const HistorySparkline: React.FC<{
  ...
  return (
    <div
      ...
      title={`${history.length} checks recorded`}
    >
...
export const DomainTable: React.FC<DomainTableProps> = ({
  ...
  const allSelected =
    domains.length > 0 && domains.every((d) => selectedIds.has(d.id));
  ...
  return (
    ...
    >
      <div className="overflow-x-auto">
        <table
          className="w-full text-left border-collapse min-w-[600px] md:min-w-0"
          role="grid"
          aria-rowcount={domains.length + 1}
        >
          <thead>
            <tr
              className="bg-zinc-900/80 border-b border-zinc-800 text-zinc-500 text-xs uppercase tracking-wider font-semibold"
              role="row"
            >
              <th
              ...
              <th className="p-4 pl-2" role="columnheader" scope="col">
                Domain
              </th>
              <th className="p-4" role="columnheader" scope="col">
                Status
              </th>
              <th className="p-4" role="columnheader" scope="col">
                Tech Stack
              ...
              <th className="p-4 text-right" role="columnheader" scope="col">
                Actions
              </th>
            </tr>
          </thead>
            ...
            {domains.map((domain) => {
              ...
              return (
                ...
                >
                    ...
                    {editingId === domain.id ? (
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-md bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400 hover:bg-green-100 dark:hover:bg-green-900/50"
                        >
                          <Check size={14} />
                        </button>
                        <button
                          ...
                          <div className="flex items-center gap-2 relative">
                            ...
                            >
                              {domain.groupId ? (
                                <GroupBadge
                                  group={groups?.find(
                                    (g) => g.id === domain.groupId,
                                  )}
                                />
                              ) : (
                                <span className="text-[10px] uppercase font-bold text-zinc-600 border border-dashed border-zinc-700 px-1.5 rounded opacity-0 group-hover:opacity-100 transition-opacity">
                                  Group
                                </span>
                              )}
                            </button>
                            ...
                            <div className="flex items-center gap-1 flex-wrap relative">
                              {domain.tags?.map((tag, i) => (
                                <span
                                  ...
                                  onClick={() =>
                                    removeTag(domain.id, domain.tags, tag)
                                  }
                  ...
                  <td className="p-4 align-middle hidden xl:table-cell">
                    {isChecking ? (
                      ...
                      <div className="flex flex-wrap gap-1 max-w-[200px]">
                        {domain.expiry?.nameServers &&
                        domain.expiry.nameServers.length > 0 ? (
                          domain.expiry.nameServers.slice(0, 2).map((ns, i) => (
                            <span
                              key={i}
                              className="px-2 py-0.5 bg-emerald-500/10 border border-emerald-500/20 rounded text-[9px] font-mono text-emerald-400 truncate max-w-[100px]"
                            >
                              {ns}
                            </span>
                          ))
                          ...
                          domain.dns.ns.slice(0, 2).map((ns, i) => (
                            <span
                              key={i}
                              className="px-2 py-0.5 bg-zinc-800 border border-zinc-700 rounded text-[9px] font-mono text-zinc-400 truncate max-w-[100px]"
                            >
                              {ns}
                            </span>
                          ))
                        ...
                        {(domain.expiry?.nameServers?.length ||
                          domain.dns?.ns?.length ||
                          0) > 2 && (
                          <span className="px-2 py-0.5 text-[9px] text-slate-400">
                            +
                            {(domain.expiry?.nameServers?.length || 0) +
                              (domain.dns?.ns?.length || 0) -
                              2}
                          </span>
                        )}
                      </div>
                    ...
                    {isChecking ? (
                      ...
                      <span className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-zinc-600">
                          (
                          {domain.lastChecked.toLocaleTimeString([], {
                            hour: "2-digit",
                            minute: "2-digit",
                          })}
                          )
                  ...
                  <td className="p-4 align-middle hidden lg:table-cell">
                    <UptimeBadge history={domain.history} />
                  </td>
